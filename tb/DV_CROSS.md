...
| `cp_termination_alignment` | `no_post_edge_frame_start`, `single_terminal_drain`, `terminal_boundary_forwarded`, `idle_after_drain` |
...
| `XC06` | `cp_run_state_transition x cp_enable_state` | No output except in legal `enabled + running` or terminal-drain combinations |
...
| `XC10` | `cp_frame_count_behavior x cp_run_state_transition` | Repeats/skips absent in legal runs, including the terminal drain transition |
...
| `XC16` | `cp_run_state_transition x cp_termination_alignment` | Terminal drain and stop-edge behavior proven in clean runs |

### 2.3 Multi-Lane / Shared-Datapath Crosses

| Cross ID | Cross | Required outcome |
|---|---|---|
| `XC17` | `cp_active_lane_count x cp_timestamp_tie_degree` | Tie behavior covered from 1 to 8 active lanes |
| `XC18` | `cp_active_lane_count x cp_merge_latency_bin` | Merge latency bounded for all active-lane populations |
| `XC19` | `cp_lane_mask_shape x cp_tie_winner_lane` | Tie policy proven for sparse and contiguous populations |
| `XC20` | `cp_delay_cfg x cp_active_lane_count` | Per-lane delay independence proven from 1 to 8 lanes |
| `XC21` | `cp_hit_mode x cp_active_lane_count` | Shared datapath proven across mode mixes and lane counts |
| `XC22` | `cp_tx_mode x cp_active_lane_count` | Shared packetizer proven across mode mixes and lane counts |
| `XC23` | `cp_lane_mask_shape x cp_merge_latency_bin` | No pathological latency for sparse or skewed masks |
| `XC24` | `cp_timestamp_tie_degree x cp_merge_latency_bin` | Tie handling stays within bounded latency |
| `XC25` | `cp_tie_winner_lane x cp_timestamp_tie_degree` | Every lane wins at least one legal tie it should win |

### 2.4 Software-Visible Crosses

| Cross ID | Cross | Required outcome |
|---|---|---|
| `XC26` | `cp_status_snapshot_kind x cp_frame_count_behavior` | Status reads remain coherent across idle/active/reset-edge sampling |
| `XC27` | `cp_status_snapshot_kind x cp_event_count_consistency` | Event-count reporting coherent across read contexts |
| `XC28` | `cp_enable_state x cp_status_snapshot_kind` | Disabled-state software view covered |
...
2. `XC01` through `XC16` must achieve full legal-bin coverage for the single-lane baseline, including the terminate-to-drain contract.
3. `XC17` through `XC25` must achieve full legal-bin coverage for the shared 8-lane architecture before the shared-datapath / area-signoff phase closes.
...
7. `bucket_frame` and `all_buckets_frame` are mandatory `DV_CROSS` baselines:
